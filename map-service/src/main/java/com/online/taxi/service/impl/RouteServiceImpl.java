...
public class RouteServiceImpl implements RouteService {
    ...
    public Distance getRoute(String carId, String city, Long startTime, Long endTime) {
        ...
        urlBuild.append("&sid=").append(amapSid);

        urlBuild.append("&terminal=").append(carId);
        urlBuild.append("&starttime=").append(startTime);
        urlBuild.append("&endtime=").append(endTime);
        urlBuild.append("&correction=driving");

        String url = urlBuild.toString();
        log.info("高德地图：查询历史轨迹里程 请求信息：" + urlBuild);
    ...
    private AmapResult<Distance> parseRoute(ResponseEntity<String> responseEntity) {
        ...
        try {
            ...
            if (result.has(AmapResultConfig.AMAP_ERRORCODE)) {
                int status = result.getInt(AmapResultConfig.AMAP_ERRORCODE);
    ...
    public Points getPointsAllPage(String carId, String city, Long startTime, Long endTime, String correction) {
        ...
        Points privateResponse = getPoints(carId, startTime, endTime, 1, pagesize, correction);

        assert privateResponse != null;
        ...
        int pageMax = (pointCount / pagesize) + 1;
        for (int i = 2; i <= pageMax; i++) {
            privateResponse = getPoints(carId, startTime, endTime, i, pagesize, correction);
    ...
    private Points getPoints(String carId, Long startTime, Long endTime, Integer page, Integer pagesize, String correction) {
        StringBuilder urlBuild = new StringBuilder();
        urlBuild.append(AmapUrlConfig.HISTORY_POINT_URL);
        urlBuild.append("?key=").append(amapKey).append("&sid=").append(amapSid);
        urlBuild.append("&terminal=").append(carId);
        urlBuild.append("&starttime=").append(startTime);
        urlBuild.append("&endtime=").append(endTime);
        urlBuild.append("&correction=").append(correction);
        urlBuild.append("&recoup=1");
        urlBuild.append("&pagesize=").append(pagesize);
        urlBuild.append("&page=").append(page);
        String url = urlBuild.toString();
