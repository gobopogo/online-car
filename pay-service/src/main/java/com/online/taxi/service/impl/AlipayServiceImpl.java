...
public class AlipayServiceImpl implements AlipayService {
    ...
    public String prePay(Integer yid, Double capital, Double giveFee, String source, Integer rechargeType, Integer orderId) {
        try {
            ...
            AlipayTradeAppPayModel model = createModel(passengerWalletRecord.getId() + "", yid, capital, giveFee, rechargeType);
    ...
    private AlipayTradeAppPayModel createModel(String rechargeId, Integer yid, Double capital, Double giveFee, Integer rechargeType) {
    ...
    public boolean callback(Map<String, String> params) {
        ...
        try {
            ...
            //充值类型
            int rechargeType = Integer.parseInt(bodyArray[3]);
    ...
    public ResponseResult<?> payResult(Integer orderId, String tradeNo) {
        try {
            ...
            if (null == subCode && code.equals(AlipayConfig.SUCCESS_CODE)) {
                ...
                assert subCode != null;
                switch (subCode) {
                    case AlipayConfig.ACQ_INVALID_PARAMETER:
                        //重新发起请求
                        return ResponseResult.success(new PayResultResponse(PayConst.RE_TRY_STATUS));
                    case AlipayConfig.ACQ_SYSTEM_ERROR:
                        //检查请求参数，修改后重新发起请求
                        return ResponseResult.success(new PayResultResponse(PayConst.RE_TRY_STATUS));
                    case AlipayConfig.ACQ_TRADE_NOT_EXIST:
